...
import { filePathToPath, groupByDirectory, listByDirectory, sortDirectoriesByDepth } from "./utils/file.js";
...
const createApp = (options) => {
	...
	for (const map of routesMap) for (const [dir, content] of Object.entries(map)) {
		...
		const findMiddleware = (directory) => Object.keys(MIDDLEWARE_FILE).find((x) => new RegExp(escapeDir(directory) + "/_middleware.tsx?").test(x));
		const isMiddlewareAppliedToAncestor = (middlewarePath, currentDir) => {
			const dirParts = currentDir.split("/");
			const ancestorDirs = [];
			for (let i = dirParts.length - 1; i > 0; i--) ancestorDirs.push(dirParts.slice(0, i).join("/"));
			return ancestorDirs.some((ancestorDir) => appliedMiddlewaresByDirectory.get(ancestorDir)?.has(middlewarePath));
		};
		const applyExtraHandlersForRouting = (handlers) => {
			const isRootLevel = !dir.replace(rootRegExp, "").includes("/");
			...
			if (Object.keys(content).length > 0 && isRootLevel && isSimpleStructure) {
				...
				Object.keys(content).forEach((filename) => {
					...
					if (path !== "/" && !path.includes("[") && !path.includes("*")) subApp.use(path, ...handlers);
				});
			}
		};
		getPaths(dir, rendererList).map((path) => {
			const renderer = RENDERER_FILE[path];
			if (renderer[IMPORTING_ISLANDS_ID]) hasIslandComponent = true;
			const rendererDefault = renderer.default;
			if (rendererDefault) {
				if (!processedRendererRequests.has(rendererDefault)) processedRendererRequests.set(rendererDefault, /* @__PURE__ */ new WeakSet());
		...
		if (!middlewareFile) {
			...
			for (let i = dirParts.length - 1; i >= 0; i--) {
				...
				if (!parentDir.includes(root)) break;
		...
		if (middlewareFile) {
			...
			if (middleware.default && shouldApply) {
				const wrappedMiddleware = middleware.default.map((mw) => {
					if (!processedMiddlewareRequests.has(mw)) processedMiddlewareRequests.set(mw, /* @__PURE__ */ new WeakSet());
				...
				if (!appliedMiddlewaresByDirectory.has(dir)) appliedMiddlewaresByDirectory.set(dir, /* @__PURE__ */ new Set());
		...
		if (errorHandler) errorHandlerMap[dir] = errorHandler;
		for (const [path, errorHandler] of Object.entries(errorHandlerMap)) if (new RegExp(`^${path}`).test(dir) && errorHandler) subApp.onError(errorHandler);
		let rootPath = getRootPath(dir);
		if (trailingSlash) rootPath = rootPath.endsWith("/") ? rootPath : rootPath + "/";
...
function getNotFoundHandler(dir, map) {
	for (const [mapDir, content] of Object.entries(map)) if (dir === mapDir) {
		const notFound = content[NOTFOUND_FILENAME];
		if (notFound) return notFound.default;
	}
}
function applyNotFound(app, dir, map, processedNotFoundResponses) {
	for (const [mapDir, content] of Object.entries(map)) if (dir === mapDir) {
		const notFound = content[NOTFOUND_FILENAME];
		if (notFound) {
			const notFoundHandler = notFound.default;
			if (notFound[IMPORTING_ISLANDS_ID]) app.use("*", (c, next) => {
